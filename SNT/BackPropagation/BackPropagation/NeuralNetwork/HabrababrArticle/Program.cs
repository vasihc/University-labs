...
namespace NeuralNetwork
{
    class InputLayer
    {
        public InputLayer(List<DataSet> datasets)
        {
            foreach (var dataset in datasets)
            {
                var x = dataset.Values;
                var y = dataset.Targets;
                var tuple = (x, y);
                _trainset.Append(tuple);
            }
        }
    ...
    class Network
    {
        ...
        InputLayer input_layer = new InputLayer(DataSet.ImportDatasets("dataTrain.txt"));
        public HiddenLayer hidden_layer = new HiddenLayer(9, 2, NeuronType.Hidden, nameof(hidden_layer));
        public OutputLayer output_layer = new OutputLayer(1, 9, NeuronType.Output, nameof(output_layer));
