    ...
    class BackPropProgram
    {
        static void Main(string[] args)
        {
            try
            {
                ...
                var dataSets = DataSet.ImportDatasets("dataTrain.txt");

                double[] xValues; ; // inputs
                double[] yValues; // outputs
                double[] tValues; // target values
                xValues = dataSets[0].Values;
                tValues = dataSets[0].Targets;
          ...
          //0.025, 0.026 };
                ...
                while (epoch < maxEpochs) // train
                {
                    ...
                    {
                        xValues = dataSets[i].Values;
                        tValues = dataSets[i].Targets;
                    }
                ...
                var testData = DataSet.ImportDatasets("dataTest.txt");

                int numTest = 15;// max 20
                xValues = testData[numTest].Values;
                tValues = testData[numTest].Targets;
                yValues = bnn.ComputeOutputs(xValues);
                Console.WriteLine("\nThe test xValues:");
                Helpers.ShowVector(xValues, 3, 9, true);

                Console.WriteLine("\nThe yValues using final weights are:");
                Helpers.ShowVector(yValues, 3, 8, true);

                Console.WriteLine("\nThe tValues:");
                Helpers.ShowVector(tValues, 3, 8, true);
